...
const PMCEVENTS: &[u8] = b"pmc-events";

/// Is this a tag we are interested in?
fn is_interested_tag(tag: &[u8]) -> bool {
    matches!(
        tag,
        TRACE_QUERY_RESULT | NODE | ROW | BACKTRACE | FRAME | PMCEVENTS
    )
}
...
impl CurrentTag {
    fn matches(&self, name: &[u8]) -> bool {
        match name {
            ...
            PMCEVENTS => matches!(self, Self::PMCEvents { .. }),
            _ => false,
        }
    }
}
...
impl BacktraceId {
    fn resolve(&self, context: &FolderCounter) -> String {
...
pub struct FolderCounter {
    ...
    /// pmc_events_id <--> Vec<event values>
    pmc_events: BTreeMap<PMCEventsId, Vec<u64>>,
}

impl Collapse for FolderCounter {
            ...
            match event {
                Event::Start(start) => {
                    ...
                    let new_state = match (self.state_backtrace.top_mut(), name) {
                        ...
                        (Some(CurrentTag::Node { .. }), ROW) => Some(CurrentTag::Row {
                            backtrace: None,
                            pmc: None,
                        }),
                        ...
                        (Some(CurrentTag::Row { .. }), PMCEVENTS) => {
                            let id = get_u64_from_attributes(ID, &attributes).map(PMCEventsId)?;
                            Some(CurrentTag::PMCEvents { id })
                        }
                ...
                Event::End(end) => {
                    ...
                    match (self.state_backtrace.top_mut(), state) {
                        ...
                        (Some(CurrentTag::Node { rows }), CurrentTag::Row { backtrace, pmc }) => {
                        ...
                        (Some(CurrentTag::Row { pmc, .. }), CurrentTag::PMCEvents { id }) => {
                            if pmc.is_some() {
                                return invalid_data_error!(
                                    "Repeated PMCEvents in xctrace output: {:?}",
                                    id
                                );
                            }
                            *pmc = Some(id);
                ...
                Event::Text(text) => {
                    // this might have bug we did not trace current tag correctly. but it work for currently counter profile xml
                    //<pmc-events id="68" fmt="(33,267), (97,198), (43,286), (87,432)">33267 97198 43286 87432</pmc-events>
                    match self.state_backtrace.top_mut() {
                        Some(CurrentTag::PMCEvents { id }) => {
                            ...
                            while let Some(part) = parts.next() {
                                if let Ok(num) = part.parse::<u64>() {
                                    ...
                                    return invalid_data_error!("Invalid PMC value: {}", part);
                                }
                            }
                            self.pmc_events.insert(*id, value);
                        }
                        _ => {}
                    }
                }
